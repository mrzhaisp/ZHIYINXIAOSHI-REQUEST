...
labesouces = ["大数据","机器人", "电子对抗", "人工智能","下一代网络","智能制造","区块链","其它"]
...
class LoadVideo:
	...
	def filter_video_name(self):
		"""拿到视频的title  如果title在数据库可查询到，则重新选择一次视频"""
		title_tuple = self.all_file_path()
		title = (title_tuple[0][1]).split(".mp4")[0]
		#查出所有数据库的已经入库的视频名称
		db_title = self.m.titles_list()
		#如果本地随机选择的视频名称 在数据库里可以查询得到，则重新调用随机挑选视频的函数
	...
	def add_video(self,cookie=None):
		"""拆分视频路径，title，上传视频接口"""
		#视频绝对路径一定要open
		try:
			colum_add_videoname_liat = self.filter_video_name()
